 ...
 *
 */
public class ConnectionDiagramCommand2 extends DefaultModuleCommandHandler {
	...
	public void actionPerformed(List<MObject> selectedElements, IModule module) {
		...
		String name = "Connections diagram 2";
		Stereotype sterConnection = session.getMetamodelExtensions().getStereotype(IINTOCPSPeerModule.MODULE_NAME,
				INTOCPSStereotypes.CONNECTIONDIAGRAM2, INTOCPSModule.getInstance().getModuleContext().getModelioServices().getMetamodelService().getMetamodel().getMClass(CompositeStructureDiagram.class));
		...
		try( ITransaction transaction = session.createTransaction(I18nMessageService.getString ("Info.Session.Create","Connection Diagram"))){
			...
			diagram = factory.createCompositeStructureDiagram(name, (ModelElement) selectedElements.get(0) ,sterConnection);
