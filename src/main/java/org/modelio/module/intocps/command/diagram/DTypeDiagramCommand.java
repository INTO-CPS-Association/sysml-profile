...
/**
 * The diagram command which handles the creation of an dtYPE
 * @author ebrosse
 *
 */
public class DTypeDiagramCommand extends DefaultBoxTool{
    ...
    public void actionPerformed(IDiagramHandle representation, IDiagramGraphic target, Rectangle rect) {
        ...
        Stereotype sterDType = session.getMetamodelExtensions().getStereotype(IINTOCPSPeerModule.MODULE_NAME,
        		INTOCPSStereotypes.DTYPE, INTOCPSModule.getInstance().getModuleContext().getModelioServices().getMetamodelService().getMetamodel().getMClass(DataType.class));

        String name = "Dtype";

        try( ITransaction transaction = session.createTransaction (I18nMessageService.getString ("Info.Session.Create","DType"))){
            MObject element = target.getElement();
            ...
            DataType datatype = session.getModel().createDataType(name, ((Package)element), sterDType);

            List<IDiagramGraphic> graph = representation.unmask(datatype, rect.x, rect.y);
