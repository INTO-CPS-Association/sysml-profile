 ...
 *
 */
public class BlockInstanceDiagramCommand extends DefaultBoxTool{
	...
	public void actionPerformed(IDiagramHandle representation, IDiagramGraphic target, Rectangle rect) {
		...
		Stereotype sterBlockInstance = session.getMetamodelExtensions().getStereotype(IINTOCPSPeerModule.MODULE_NAME,
				INTOCPSStereotypes.BLOCKINSTANCE, INTOCPSModule.getInstance().getModuleContext().getModelioServices().getMetamodelService().getMetamodel().getMClass(Instance.class));

		String name = "instance";

		Instance blockInstance = null;

		try( ITransaction transaction = session.createTransaction (I18nMessageService.getString ("Info.Session.Create","Block Instance"))){
			MObject element = target.getElement();
			...
			if (element instanceof org.modelio.metamodel.uml.statik.Package){
				blockInstance = session.getModel().createInstance();
				blockInstance.setOwner((org.modelio.metamodel.uml.statik.Package) element);
			}else{
				blockInstance = session.getModel().createBindableInstance();
				((BindableInstance) blockInstance).setCluster((Instance) element);
			}

			session.getModel().getDefaultNameService().setDefaultName(blockInstance, name);
			blockInstance.getExtension().add(sterBlockInstance);

			List<IDiagramGraphic> graph = representation.unmask(blockInstance, rect.x, rect.y);
