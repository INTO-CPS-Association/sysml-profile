...
/**
 * The diagram command which handles the creation of a INTOCPS port
 *
 * @author ebrosse
 *
 */
public class DSEScriptParameterDiagramCommand extends DefaultBoxTool {

    /**
     * Default constructor
     */
    public DSEScriptParameterDiagramCommand() {
        super();
    }
    ...
    public boolean acceptElement(IDiagramHandle representation, IDiagramGraphic target) {
        ...
        return (element.getStatus().isModifiable()
                && ((element instanceof Class) || (element instanceof Instance)));
    }
    ...
    public void actionPerformed(IDiagramHandle representation, IDiagramGraphic target, Rectangle rect) {
        ...
        Stereotype ster = session.getMetamodelExtensions().getStereotype(IINTOCPSPeerModule.MODULE_NAME,
                INTOCPSStereotypes.SCRIPTPARAMETER_PORT, INTOCPSModule.getInstance().getModuleContext().getModelioServices().getMetamodelService().getMetamodel().getMClass(Port.class));
        try (ITransaction transaction = session.createTransaction(I18nMessageService.getString("Info.Session.Create", "DSE Script Parametert"))) {
