 ...
 *
 */
public class StrtTypeDiagramCommand extends DefaultBoxTool{

    /**
     * Default constructor
     */
    public StrtTypeDiagramCommand() {
        super();
    }
    ...
    public void actionPerformed(IDiagramHandle representation, IDiagramGraphic target, Rectangle rect) {
        ...
        try( ITransaction transaction = session.createTransaction (I18nMessageService.getString ("Info.Session.Create","StrtType"))){
            MObject element = target.getElement();

            Stereotype sterStrtType = session.getMetamodelExtensions().getStereotype(IINTOCPSPeerModule.MODULE_NAME,
            		INTOCPSStereotypes.STRTTYPE, INTOCPSModule.getInstance().getModuleContext().getModelioServices().getMetamodelService().getMetamodel().getMClass(Interface.class));

            String name = "StrtType";

            if (element instanceof AbstractDiagram) {
                element = ((AbstractDiagram) element).getOrigin();
            }

            Interface strttype = session.getModel().createInterface(name, ((Package)element), sterStrtType);

            List<IDiagramGraphic> graph = representation.unmask(strttype, rect.x, rect.y);
