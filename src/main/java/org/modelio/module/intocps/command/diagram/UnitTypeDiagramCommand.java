 ...
 *
 */
public class UnitTypeDiagramCommand extends DefaultBoxTool{
	...
	public void actionPerformed(IDiagramHandle representation, IDiagramGraphic target, Rectangle rect) {
		...
		try( ITransaction transaction = session.createTransaction (I18nMessageService.getString ("Info.Session.Create","UnitType"))){

			Stereotype sterUnitType = session.getMetamodelExtensions().getStereotype(IINTOCPSPeerModule.MODULE_NAME,
					INTOCPSStereotypes.UNITTYPE, INTOCPSModule.getInstance().getModuleContext().getModelioServices().getMetamodelService().getMetamodel().getMClass(DataType.class));
			...
			DataType unittype = session.getModel().createDataType(name, ((Package)element), sterUnitType);
			List<IDiagramGraphic> graph = representation.unmask(unittype, rect.x, rect.y);
