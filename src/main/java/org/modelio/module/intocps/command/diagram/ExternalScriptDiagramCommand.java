 ...
 *
 */
public class ExternalScriptDiagramCommand extends DefaultBoxTool{
	...
	public void actionPerformed(IDiagramHandle representation, IDiagramGraphic target, Rectangle rect) {
		...
		Stereotype sterBlockInstance = session.getMetamodelExtensions().getStereotype(IINTOCPSPeerModule.MODULE_NAME,
				INTOCPSStereotypes.EXTERNALSCRIPT, INTOCPSModule.getInstance().getModuleContext().getModelioServices().getMetamodelService().getMetamodel().getMClass(org.modelio.metamodel.uml.statik.Class.class));

		String name = "Script";


		try( ITransaction transaction = session.createTransaction (I18nMessageService.getString ("Info.Session.Create","External Script"))){
			MObject element = target.getElement();
