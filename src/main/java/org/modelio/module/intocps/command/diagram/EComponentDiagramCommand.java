 ...
 *
 */
public class EComponentDiagramCommand extends DefaultBoxTool{
    ...
    public void actionPerformed(IDiagramHandle representation, IDiagramGraphic target, Rectangle rect) {
        ...
        Stereotype sterEComponent = session.getMetamodelExtensions().getStereotype(IINTOCPSPeerModule.MODULE_NAME,
                INTOCPSStereotypes.ECOMPONENT, INTOCPSModule.getInstance().getModuleContext().getModelioServices().getMetamodelService().getMetamodel().getMClass(org.modelio.metamodel.uml.statik.Class.class));

        Stereotype sterSubSystem = session.getMetamodelExtensions().getStereotype(IINTOCPSPeerModule.MODULE_NAME,
                INTOCPSStereotypes.SUBSYSTEM, INTOCPSModule.getInstance().getModuleContext().getModelioServices().getMetamodelService().getMetamodel().getMClass(org.modelio.metamodel.uml.statik.Class.class));


        String name = "EComponent";

        org.modelio.metamodel.uml.statik.Class system = null;

        try( ITransaction transaction = session.createTransaction (I18nMessageService.getString ("Info.Session.Create","EComponent"))){
            MObject element = target.getElement();
            ...
            if (system != null){
                ...
                system.getExtension().add(sterEComponent);
                system.getExtension().add(sterSubSystem);

                ModelUtils.addValue(IINTOCPSPeerModule.MODULE_NAME, INTOCPSTagTypes.COMPONENT_KIND, EComponentKind.SubSystem.name(), system);
