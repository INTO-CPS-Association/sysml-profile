 ...
 *
 */
public class CComponentDiagramCommand extends DefaultBoxTool{
    ...
    public void actionPerformed(IDiagramHandle representation, IDiagramGraphic target, Rectangle rect) {
        ...
        Stereotype sterBlockInstance = session.getMetamodelExtensions().getStereotype(IINTOCPSPeerModule.MODULE_NAME,
                INTOCPSStereotypes.CCOMPONENT, INTOCPSModule.getInstance().getModuleContext().getModelioServices().getMetamodelService().getMetamodel().getMClass(org.modelio.metamodel.uml.statik.Class.class));

        String name = "CComponent";

        org.modelio.metamodel.uml.statik.Class system = null;

        try( ITransaction transaction = session.createTransaction (I18nMessageService.getString ("Info.Session.Create","CComponent"))){
            MObject element = target.getElement();
            ...
            if (system != null){
                ...
                system.getExtension().add(sterBlockInstance);
                ModelUtils.addValue(IINTOCPSPeerModule.MODULE_NAME, INTOCPSTagTypes.COMPONENT_KIND, CComponentKind.Composition.name(), system);
